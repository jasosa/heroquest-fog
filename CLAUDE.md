...
| File | Purpose |
|---|---|
| `src/heroquest-fog.jsx` | Main app (~750 lines): all components, `useGameState` hook, screen router |
| `src/QuestLibrary.jsx` | Quest library screen (two-panel: books sidebar + quest cards grid) |
| `src/questStorage.js` | Pure localStorage CRUD for quest books and quests |
...
Key functions: `createQuestBook`, `deleteQuestBook` (cascades), `createQuest`, `persistQuest` (upsert), `deleteQuest`, `loadQuests`, `loadQuestBooks`.
