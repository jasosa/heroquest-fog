...
(async () => {
  ...
  await page.locator('input[placeholder="Book title"]').fill('Trial Book');
  ...
  log('\n=== FLOW 2: Create Quest → opens game screen directly ===');
  await page.locator('text=Trial Book').first().click();
  await page.waitForTimeout(200);
  await page.click('button:has-text("New Quest")');
  await page.waitForTimeout(400);
  await page.locator('input[placeholder="Quest title"]').fill('Quest One');
  // "Create & Edit" goes straight to game screen
  await page.locator('button').filter({ hasText: /Create.*Edit/i }).first().click();
  await page.waitForTimeout(800);
  const p2 = await ss(page, '02_game_screen');
  const gameBtns = await page.locator('button').allTextContents();
  log('Game screen buttons: ' + JSON.stringify(gameBtns));
  // Verify we're in game screen (has Library button and Play/Edit buttons)
  const hasLibBtn = await page.locator('button').filter({ hasText: /library/i }).count();
  if (hasLibBtn === 0) {
    issue('High', 'Create & Edit Quest', '1.New Quest 2.Fill title 3.Create & Edit', 'Game screen with Library button', 'Not in game screen', p2);
  } else { log('OK - In game screen after Create & Edit'); }
