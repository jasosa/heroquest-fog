...
- **Scope correction:** only `board`, `board2`, `board3` actually exist/are selectable (`public/board4.png` does not exist; `CALIBRATE_MAPS` in `GameScreen.jsx` and the tileset buttons in `Sidebar.jsx` only register 3 tilesets). The `board4` mentioned in the original description and in `docs/architecture/pieces.md`/`data-model.md` isn't wired up anywhere — narrow this issue's scope to the 3 real tilesets, or treat `board4` as aspirational/future.
- **Ruled out:** `TokenOverlay.jsx`'s `imageScale`/`resolveScale` rendering. Centering is done via a symmetric bounding-box midpoint (`translate(-50%,-50%)`-equivalent) — a smaller scale shrinks a piece evenly around its existing center, it cannot shift that center. Not a contributing factor.
- **Ruled out:** the logical grid itself (`BoardCell.jsx`/`BoardGrid.jsx`/fog SVG). All derive from the same `CELL=37` constant, tiled edge-to-edge with no cumulative gaps — internally consistent, not a source of drift.
- **Calibration is opt-in only, not seeded.** `loadCalibration()` returns `{}` on a fresh browser — there's no default/shipped calibration data anywhere in the repo. This means on any install where a developer hasn't manually run `MapCalibrator` for a given tileset, that tileset falls back to the uncalibrated formula (`col*CELL+CELL/2, row*CELL+CELL/2`), which is centered on the logical grid *by definition*. Since `Sidebar.jsx` has no calibration UI at all (only reachable via the separate `"calibrate"` screen), it's unlikely most users have ever populated this.
- **Leading candidate (unconfirmed, needs pixel measurement):** the `board.png`/`board2.png` source assets' baked-in grid lines may not land on exact integer-pixel boundaries for a clean 26×19 division of the image's native size — `backgroundSize: 100% 100%` would stretch that mismatch into a per-tileset (and possibly non-uniform, cell-by-cell) centering drift, independent of calibration entirely. Visual inspection of the PNGs during the spike showed what looked like two slightly-offset overlaid grid line sets, consistent with this — but this could not be distinguished from a rendering/downsampling artifact of the image-viewing tool used during the spike (no pixel-measurement tooling was available in that investigation).
- **What determines the actual fix:** whether the drift (if real) is a *constant* per-tileset offset (simple fix — a small correction constant in the uncalibrated fallback formula, keyed by `bgImage`) or *non-uniform*/local distortion baked into the art (harder fix — re-export the source assets, or ship a default calibration per tileset instead of relying on opt-in localStorage).

**Next step before this can be planned:** a live visual comparison is needed — place identical pieces at the same grid coordinates across all three tilesets at high zoom and screenshot/compare, or measure the PNGs' exact pixel dimensions and grid-line coordinates directly in an image editor. Neither was possible from this (headless/background) session. Left `in_progress`, not `not_started`, since the investigation is complete and the remaining step just needs a live-viewing environment.
