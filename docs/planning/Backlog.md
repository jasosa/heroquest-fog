...
**Spike findings (2026-08-16, static code analysis):** only `board`/`board2`/`board3` actually exist (`board4` referenced in the original description and in `docs/architecture/pieces.md`/`data-model.md` isn't wired up anywhere — scope corrected). Ruled out `TokenOverlay.jsx`'s `imageScale`/`resolveScale` rendering (scale-independent, symmetric centering) and the logical grid itself (`BoardCell.jsx`/`BoardGrid.jsx`, internally consistent, all derived from `CELL=37`) as contributing factors via code reading. Calibration confirmed opt-in only, not seeded — most real users are on the uncalibrated fallback path. Leading candidate at that point (unconfirmed): the PNG assets' baked-in grid lines not landing on exact pixel boundaries for a 26×19 division.

**Live verification 2026-08-16 (via a connected browser session — conclusive):** all three tilesets' PNGs measured at exactly 963×704px (identical). Placed a Goblin (image piece, `imageScale` 1.2 on `board`/`board2`, 1.0 on `board3`) and a Search Marker (non-image piece, `translate(-50%,-50%)` centering path) in the running app, then measured each piece's rendered center against its target cell's mathematical center via `getBoundingClientRect()` — with zero calibration configured (confirmed via `localStorage.hq_calibration`, all three tilesets `ready:false`). Result: **offset was 0 pixels (down to floating-point noise, ~0.00001px) on all three tilesets, for both the image-piece and non-image-marker rendering paths.** This directly refutes the "PNG grid misalignment" hypothesis and confirms there is no piece-centering bug in the code.

**Conclusion:** the original visual complaint is almost certainly a *perceptual* effect, not a positioning defect — pieces with `imageScale > 1` (most monsters on `board`/`board2`) are deliberately oversized and overflow into neighboring cells at the edges, and the character artwork's own visual weight within its square canvas isn't necessarily centered (e.g. a goblin's head/horns drawn off-center within its own image bounds) even though the bounding box anchor is mathematically exact. This reads as "off-center" to the eye without any code being wrong. No code fix needed. If tighter *visual* (not mathematical) centering is wanted, the options are art-asset changes (re-crop piece images so their visual mass is centered in their own canvas) or reducing `imageScale` toward 1.0 — a polish/art-direction call, not a bug fix; not opened as a follow-up item unless requested.

### [FEAT-040] Application UI internationalization (English/Spanish)
Priority: medium
Status: done
Complexity: high
Description: The application's own UI chrome — Quest Library, Sidebar, dialogs, buttons, labels, tooltips, section headers, error/status messages, etc. — should support English and Spanish, built on a maintainable internationalization system (structured translation keys / a lightweight i18n library, not hardcoded strings scattered across ~40+ components as today) so additional languages can be added later without a rewrite. Add a language switcher (persisted, e.g. in `localStorage`, alongside the existing `hq_calibration`/`hq_quests`/`hq_quest_books` keys) letting the user pick English or Spanish for the app UI. Distinct from FEAT-041 (quest content translation) — this item covers only the application's own interface text, not DM-authored quest content.

Implementation: hand-rolled i18n, zero new npm dependency. A pure `t(dict, key, vars)` lookup+interpolation function (`src/shared/i18n/t.js`), English/Spanish JSON dictionaries nested by feature area (`src/shared/i18n/en.json`/`es.json`), and `LocaleContext`/`I18nProvider`/`useI18n()` (the first and only React Context in this codebase) exposing `{ locale, setLocale, t }`, persisted to a new `hq_locale` localStorage key. First-load default detects `navigator.language` (Spanish if it starts with `"es"`, else English) and persists the resolved value immediately so it's deterministic thereafter. `useI18n()` has a safe no-provider English fallback — required because several existing tests (`GameScreen.tooltip.test.jsx`/`GameScreen.pan.test.jsx`/`GameScreen.fitZoom.test.jsx`) render `HeroQuestFog` directly, bypassing `App.jsx`'s provider-wrapping.

Scope (deliberately incremental, not all ~40+ components): fully migrated `QuestLibrary.jsx`, `Sidebar.jsx`, and `TrapConfigDialog.jsx` as a representative slice, plus a synced two-instance language switcher (Library top bar + Sidebar header, both reading/writing the same Context). Everything else in the app — `EditPanel.jsx`, `src/shared/pieces.js` catalogue labels, `GameScreen.jsx`'s own dialogs, every other board dialog — intentionally left hardcoded English, tracked as follow-up work rather than scoped into this item.

Went through four Review Cycle iterations, three of which each found one more untranslated string specific to `QuestLibrary.jsx`: a quest-card "Delete" button label, a "New" badge on recent quest cards, and — most substantively — the quest-import error message, which was structurally unreachable for translation (`err.message ?? t(fallback)` always short-circuited on the always-truthy `err.message`, so the translated fallback was dead code). The last one required a real fix, not just a string swap: `questStorage.js`'s `importQuestFromJson` now attaches a stable, locale-agnostic `.code` to thrown errors instead of only an English message, and `QuestLibrary.jsx` resolves the display string via `t()` at render time (not catch-time) so the message also updates correctly if the user switches locale after a failed import — a second bug found and fixed alongside the first. Per CLAUDE.md's 3-iteration rule, the branch was parked (pushed, unmerged, Status left `in_progress`) and flagged to the user after the third finding rather than auto-continuing; the user reviewed the specific gap and explicitly authorized a fourth fix cycle, which passed review cleanly.
